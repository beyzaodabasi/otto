...
@section('customjs')
    ...
    <script>
        ...
        // "Ürün Ekle" butonuna tıklandığında modalı açıyoruz.
        $('#openProductModal').click(function () {
            $('#newProductModal').modal('show');
        });

        // Modal tamamen açıldığında DataTable'ı initialize veya güncelliyoruz.
        $('#newProductModal').on('shown.bs.modal', function () {
            if (!$.fn.DataTable.isDataTable('#products')) {
                productTable = $('#products').DataTable({
                    ...
                    columns: [
                        { data: 'specialCodeDescription', className: 'text-center' },
                        { data: 'type', className: 'text-center' },
                        { data: 'code', className: 'text-center' },
                        { data: 'description', className: 'text-center' },
                        { data: 'groupCode', className: 'text-center' },
                        { data: 'mainUnit', className: 'text-center' },
                        {
                            data: null,
                            render: function (data, type, row) {
                                return `<button class="btn btn-sm btn-success select-product" data-product='${JSON.stringify(row)}'>Seç</button>`;
                            },
        ...
        // Ürün seçme işlemi: Event delegation ile document üzerinden yakalama
        $(document).on('click', '.select-product', function () {
            ...
            try {
                ...
                console.error("Ürün verileri çözümlenemedi:", e);
                return;
            }

            let quantity = prompt("Lütfen adet giriniz:", "1");
            if (quantity !== null && quantity > 0) {
                let duplicateFound = false;

                $("#qr-table-body tr").each(function () {
                    let existingCode = $(this).find("td").eq(2).text().trim();
                    if (existingCode === product.id) {
                        duplicateFound = true;
                        let quantityCell = $(this).find("td").eq(6);
                        ...
                        let inputIndex = $(this).attr('data-index');
                        let hiddenInput = $(`input[name='products[${inputIndex}][quantity]']`);
                        hiddenInput.val(newQuantity);
                    }
                });

                if (!duplicateFound) {
                    let productIndex = $("#qr-table-body tr").length; // Ürün sayısını index olarak belirliyoruz.

                    let newRow = `
                                <tr data-index="${productIndex}">
                                    <td>${product.specialCodeDescription ? product.specialCodeDescription : ""}</td>
                                    <td>${product.type ? product.type : ""}</td>
                                    <td>${product.code ? product.code : ""}</td>
                                    <td>${product.description ? product.description : ""}</td>
                                    <td>${product.groupCode ? product.groupCode : ""}</td>
                                    <td>${product.mainUnit ? product.mainUnit : ""}</td>
                    ...
                    // Form içine gizli input ekleme (index bazlı)
                    let hiddenInputs = `
        ...
        // Ürün silme işlemi ve ilgili inputları da kaldırma
        $('#qr-table-body').on('click', '.remove-product', function () {
            let row = $(this).closest('tr');
            let inputIndex = row.attr('data-index');
            row.remove();
